...
tk.Label(window,text="Set alarm temperature (HIGHEST)").grid()
lbl=Entry(window,textvariable=setT,width=5)
lbl.grid()
setT.set(-60)


tk.Label(window,text="Set alarm temperature (LOWEST)").grid()
...
def start_program():
    ...
    global setTTL
    with open('/home/pi/alarmsetL.txt','r') as tempsetL:
        setTTL=json.load(tempsetL)        
    def read_temp_raw():
    ...
    def read_temp():
        ...
        if equals_pos !=-1:
            ...
            if temp_c>=setTT or temp_c<=setTTL: 
